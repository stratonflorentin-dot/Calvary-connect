-- FINAL FIXED VERSION - Accounting Integration
...
-- ============================================
-- FUNCTION: Process Expense with Table Check
-- ============================================
...
DECLARE
    ...
    v_journal_id UUID;
BEGIN
    -- Try to get expense - will fail gracefully if table doesn't exist
    BEGIN
    ...
    v_journal_id := create_expense_journal_entry(
        p_expense_id, v_expense.expense_number, v_expense.category,
        v_expense.amount, v_expense.description, COALESCE(v_expense.payment_method, 'cash')
    );
...
-- ============================================
-- FUNCTION: Process Invoice with Table Check
-- ============================================
...
BEGIN
    ...
    -- Try to get trip revenue account
    BEGIN
        SELECT revenue_account_code INTO v_revenue_account
        FROM trip_accounting WHERE trip_id = v_invoice.trip_id;
    EXCEPTION WHEN undefined_table THEN
    ...
    v_journal_id := create_invoice_journal_entry(
        p_invoice_id, v_invoice.invoice_number, v_invoice.client_name,
        v_invoice.amount, v_invoice.vat_amount, v_invoice.total_amount,
        v_invoice.description, v_revenue_account
    );
