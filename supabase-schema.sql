...
CREATE POLICY "CEO and OPERATOR can manage trips" ON trips FOR ALL USING (auth.jwt()->'role' in ('CEO', 'OPERATOR'));

-- Expenses
CREATE POLICY "Authenticated users can view expenses" ON expenses FOR SELECT USING (auth.role() IS NOT NULL);
CREATE POLICY "CEO and OPERATOR can manage expenses" ON expenses FOR ALL USING (auth.jwt()->'role' in ('CEO', 'OPERATOR'));
...
CREATE POLICY "CEO and OPERATOR can manage maintenance requests" ON maintenance_requests FOR ALL USING (auth.jwt()->'role' in ('CEO', 'OPERATOR'));

-- Spare Parts
CREATE POLICY "Authenticated users can view spare parts" ON spare_parts FOR SELECT USING (auth.role() IS NOT NULL);
CREATE POLICY "CEO and OPERATOR can manage spare parts" ON spare_parts FOR ALL USING (auth.jwt()->'role' in ('CEO', 'OPERATOR'));

-- Parts Requests
CREATE POLICY "Authenticated users can view parts requests" ON parts_requests FOR SELECT USING (auth.role() IS NOT NULL);
CREATE POLICY "CEO and OPERATOR can manage parts requests" ON parts_requests FOR ALL USING (auth.jwt()->'role' in ('CEO', 'OPERATOR'));

-- Allowances
CREATE POLICY "Authenticated users can view allowances" ON allowances FOR SELECT USING (auth.role() IS NOT NULL);
CREATE POLICY "CEO and OPERATOR can manage allowances" ON allowances FOR ALL USING (auth.jwt()->'role' in ('CEO', 'OPERATOR'));

-- Insert sample data
INSERT INTO vehicles (plate_number, make, model, year, type, status) VALUES
('ABC-123', 'Volvo', 'FH16', 2022, 'TRUCK', 'ACTIVE'),
('DEF-456', 'Great Dane', 'Drop Deck', 2021, 'TRAILER', 'ACTIVE'),
('GHI-789', 'Ford', 'Explorer', 2023, 'ESCORT_CAR', 'ACTIVE'),
('JKL-012', 'Generic', 'Hose Kit', 2022, 'HOSE', 'ACTIVE')
