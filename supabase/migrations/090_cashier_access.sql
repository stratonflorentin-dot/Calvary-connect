-- Grants the new CASHIER role access to what Petty Cash actually needs.
-- post_bank_transaction has its own hardcoded role check independent of
-- any RLS policy — Petty Cash's "credit" (box top-up) entries call it
-- directly, and without this it would reject every cashier-recorded
-- top-up with "Only CEO/ADMIN/ACCOUNTANT may post bank transactions."
...
-- Journal Entries page and general read access for the Cashier — a cashier
-- needs to see the entries their own petty cash debits/credits produced,
-- and the page's own COA account picker reads the accounts table directly.
-- RLS on journal_entries/journal_entry_lines/accounts could not be
-- inspected live while writing this migration (Supabase MCP connection
-- was down) — if a cashier still gets "permission denied" on these tables
-- after this runs, that's the next place to look; grant read (and, for
-- journal_entries/journal_entry_lines, insert — Petty Cash's debit path
-- writes these directly before calling post_journal_entry) to CASHIER
-- alongside whatever roles those policies already allow.
