-- post_bank_transaction() (090_cashier_access.sql) captures v_txn via
-- `RETURNING * INTO v_txn` at INSERT time, then — only when a JE is also
-- created — does a separate `UPDATE bank_transactions SET journal_entry_id
-- = v_entry_id WHERE id = v_txn.id`. The real row in the table gets the
-- link; the in-memory v_txn record returned to the caller never does, so
-- every caller that reads the RPC's own return value (journal.ts's
-- postJournalEntry(), and this session's transfer_funds() / the payments
-- page fix in 125_payment_bank_transaction_linking.sql) has always seen
-- journal_entry_id: null even when the JE was created successfully.
-- Caught live while smoke-testing transfer_funds() on production data — a
-- real journal_entries row existed, but transfer_funds()'s own null-check
-- on the returned value treated it as a failure and rolled the whole
-- transfer back. (No real payments had gone through the 125 code path yet
-- when this was found, so nothing needed backfilling there.)
--
-- Fix: mirror the update onto the in-memory record before returning it.
