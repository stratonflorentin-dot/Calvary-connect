-- ==========================================================================
-- LEGACY BASE SCHEMA — bootstrap for a fresh local database ONLY.
-- ==========================================================================
--
-- Every migration numbered 001 and above in this folder only ever ALTERs
-- or adds RLS policies to tables like accounts, user_profiles, vehicles,
-- trips, journal_entries — none of them contain that table's original
-- CREATE TABLE. Those tables were created directly against production at
-- some point via database/migrations/001-master-production-setup.sql
-- (or a close variant of it), run by hand outside any tracked migration
-- system. That means `supabase db reset` could never produce a working
-- local replica on its own — it would fail on the very first ALTER TABLE
-- referencing a table that was never created.
--
-- This file is that original script, copied in as migration 000 so the
-- numbered sequence is self-sufficient: 000 creates the base tables with
-- their original (often wide-open) policies, then 001–043 correctly
-- ALTER/fix them forward to today's actual state, in the same order they
-- were originally applied. One real gap fixed on copy: the original file
-- creates a POLICY on `allowances` without ever creating that table —
-- copied here from database/migrations/005-supabase-schema.sql, the
-- schema variant that matches what src/app/allowances/actions.ts and
-- src/components/dashboard/hr-view.tsx actually read/write.
--
-- KNOWN INCOMPLETE: Phase 2/3 of this project's audit found 16 tables
-- that exist live in production with NO CREATE TABLE statement anywhere
-- in this entire repository (database/migrations, database/patches, or
-- supabase/migrations) — they were created by hand in the Supabase
-- dashboard at some point: rate_sheets, ai_agent_messages, ai_agent_runs,
...
-- truck_insurance, vehicle_deletion_audit, vehicle_service_records. A
-- fresh local reset genuinely cannot recreate these — their real column
-- shape isn't recorded anywhere. Tests touching them will need either a
-- follow-up migration (once someone inspects the live columns directly)
-- or to be skipped locally.
--
-- DANGER — read before ever running this file anywhere but
-- `supabase db reset`: the block below headed "DROP ALL EXISTING
-- POLICIES FOR CLEAN RESET" iterates every policy in the public and
-- storage schemas and drops it. That is only safe here because
-- `supabase db reset` always tears down and recreates the entire local
-- database before reapplying migrations 000 → latest in order, so this
-- block only ever runs against a database that has nothing in it yet.
-- Running this file by hand against an already-migrated database (local
-- or, worse, production) would delete every real RLS policy this project
-- has, including every fix from migrations 006 through 043.
--
-- ==========================================================================
...
-- Allowances — NOT in the original 001-master-production-setup.sql (which
-- creates a POLICY on this table without ever creating it — see header).
-- Schema copied from database/migrations/005-supabase-schema.sql, the
-- variant that matches live application usage.
...
-- Safe ONLY because this is migration 000 on a freshly-torn-down
-- `supabase db reset` database — see the file header.
DO $$
DECLARE
    r RECORD;
BEGIN
...
-- These are deliberately wide-open "USING (true)" policies, matching what
-- was actually live in production at this point in its history — migrations
-- 034/038/040/043 below correctly narrow every one of these down to real
-- role-scoped policies, in the same order they were originally applied.
...
-- KNOWN BUG, copied faithfully rather than silently fixed here (000's
-- job is historical fidelity — see file header): this inserts into
-- `notifications` using columns (sender_id, sender_name, category,
-- action_url) that don't match the table's real schema (id, user_id,
-- type, title, message, data, is_read, created_at — see
-- supabase/migrations/20240611_hr_meetings.sql /
-- 001_erp_workflow_tables.sql). src/services/audit-service.ts calls this
-- RPC live (AuditService, wrapped in try/catch that only console.errors),
-- so CEO/ADMIN have likely never actually received a single one of these
-- change notifications in production. Worth its own fix migration —
-- flagged, not touched, while this file is just reproducing bootstrap
-- history for local testing.
