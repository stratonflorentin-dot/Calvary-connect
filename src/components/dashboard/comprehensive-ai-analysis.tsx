...
export default function ComprehensiveAIAnalysis() {
  const { t } = useLanguage();
  ...
  const aiInsights = useMemo(() => {
    ...
    if (fleetUtilization < 50 && vehicles.length > 0) {
      insights.push(
        `Low fleet utilization detected (${fleetUtilization.toFixed(1)}%). Consider optimizing trip assignments.`,
      );
    }

    if (netProfit < 0) {
      insights.push(
        `Operating at a loss of ${format(Math.abs(netProfit))}. Review expense categories.`,
      );
    }

    if (profitMargin > 20) {
      insights.push(
        `Strong profit margin of ${profitMargin.toFixed(1)}%. Consider fleet expansion.`,
      );
    }

    if (crossBorderTrips > 0) {
      insights.push(
        `${crossBorderTrips} active cross-border trips. Ensure compliance with regional regulations.`,
      );
    }

    if (coldChainTrips > 0) {
      insights.push(
        `${coldChainTrips} cold chain trips in progress. Monitor temperature-sensitive cargo.`,
      );
    }

    if (maintenanceVehicles > vehicles.length * 0.3) {
      insights.push(
        `High maintenance ratio (${maintenanceVehicles} vehicles). Review fleet age.`,
      );
    }
    ...
    if (completedTrips.length > 0) {
      ...
      insights.push(
        `Avg revenue/trip: ${format(avgRevenue)} vs cost/trip: ${format(costPerTrip)}.`,
      );
    }

    if (insights.length === 0) {
      insights.push("Fleet operations running within normal parameters.");
    }
      ...
      ? [
          {
            ...
            description: `Fleet operating at a loss of ${format(Math.abs(netProfit))}.`,
  ...
  const activities = [
    {
      ...
      description: `Processed ${completedTrips.length} trips and ${vehicles.length} vehicles`,
      time: "Just now",
      icon: Sparkles,
      color: "bg-blue-500",
    },
    {
      ...
      description: `Monthly revenue: ${format(totalRevenue)}`,
      time: "Based on data",
      icon: DollarSign,
      color: "bg-green-500",
    },
    {
      ...
      description: `${fleetUtilization.toFixed(1)}% fleet utilization`,
      time: "Based on data",
      icon: TrendingUp,
      color: "bg-amber-500",
    },
