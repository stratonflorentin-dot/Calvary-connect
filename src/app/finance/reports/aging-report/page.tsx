    ...
    <div className="space-y-6 pb-8">
          ...
          <div className="flex items-center rounded-xl border border-slate-200 bg-white p-1">
            <button
              ...
              className={cn("px-3 py-1.5 text-xs font-bold rounded-lg transition-colors flex items-center gap-1",
                kind === "ar" ? "bg-indigo-600 text-white shadow-sm" : "text-slate-600 hover:text-slate-800")}
            >
            ...
            <button
              onClick={() => setKind("ap")}
              className={cn("px-3 py-1.5 text-xs font-bold rounded-lg transition-colors flex items-center gap-1",
                kind === "ap" ? "bg-orange-600 text-white shadow-sm" : "text-slate-600 hover:text-slate-800")}
            >
            ...
            <div key={`strip-${cur}`} className="bg-white border border-slate-200 rounded-2xl p-5">
              <div className="flex items-baseline justify-between mb-4">
                <div className="flex items-baseline gap-2">
                  ...
                  <h2 className="text-sm font-black text-slate-800">Portfolio distribution</h2>
                </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
                {AGING_BUCKETS.map((b) => (
                  <div key={b.key} className={cn("rounded-xl border p-4", b.color)}>
                    ...
                    <p className="text-[10px] text-slate-500 mt-0.5">{s.counts[b.key]} open</p>
                  </div>
                ))}
              </div>
            </div>
      ...
      {loading ? (
        ...
        currencies.map((cur) => {
          ...
          return (
            <div key={`party-${cur}`} className="bg-white border border-slate-200 rounded-2xl overflow-hidden">
              <div className="px-5 py-4 border-b border-slate-100 flex items-baseline justify-between">
                <div>
                  <h2 className="text-sm font-black text-slate-800">
                    By {kind === "ar" ? "Customer" : "Vendor"}
                    <span className="ml-2 text-[10px] font-black uppercase tracking-widest px-2 py-0.5 rounded-full bg-slate-100 text-slate-600">{cur}</span>
                  </h2>
                  <p className="text-xs text-slate-500">Aggregated open balances in {cur}</p>
                </div>
              </div>
              <table className="w-full text-sm">
                ...
                <tbody>
                  {parties.map((p) => (
                    <tr key={p.party} className="border-t border-slate-100 hover:bg-slate-50">
                      <td className="px-5 py-2 font-bold text-slate-800">{p.party}</td>
                      {AGING_BUCKETS.map((b) => (
                        <td key={b.key} className="px-4 py-2 text-right text-slate-600">
                          {p.buckets[b.key] > 0 ? fmt(p.buckets[b.key], cur) : "—"}
                        </td>
                      ))}
                      <td className="px-5 py-2 text-right font-black text-slate-900">{fmt(p.total, cur)}</td>
                    </tr>
                  ))}
                  <tr className="border-t-2 border-slate-200 bg-slate-50 font-black">
                    <td className="px-5 py-2 text-slate-800">Total ({cur})</td>
                    {AGING_BUCKETS.map((b) => (
                      <td key={b.key} className="px-4 py-2 text-right text-slate-800">{fmt(s.totals[b.key], cur)}</td>
                    ))}
                    <td className="px-5 py-2 text-right text-slate-900">{fmt(s.totalOutstanding, cur)}</td>
                  </tr>
                </tbody>
              </table>
            </div>
      ...
      {loading ? null : currencies.map((cur) => {
        ...
        return (
          <div key={`lines-${cur}`} className="bg-white border border-slate-200 rounded-2xl overflow-hidden">
            <div className="px-5 py-4 border-b border-slate-100">
              <h2 className="text-sm font-black text-slate-800">
                Open Line Items
                <span className="ml-2 text-[10px] font-black uppercase tracking-widest px-2 py-0.5 rounded-full bg-slate-100 text-slate-600">{cur}</span>
              </h2>
              <p className="text-xs text-slate-500">Sorted by days overdue, oldest first · {list.length} line{list.length === 1 ? "" : "s"}</p>
            </div>
                  ...
                  {list.map((r) => {
                    ...
                    return (
                      <tr key={r.id} className="border-t border-slate-100 hover:bg-slate-50">
                        <td className="px-5 py-2 font-medium text-slate-800">{r.party}</td>
                        <td className="px-4 py-2 font-mono text-xs">{r.invoice_number}</td>
                        <td className="px-4 py-2 text-slate-500 text-xs">{r.due_date ? new Date(r.due_date).toLocaleDateString() : "—"}</td>
                        <td className={cn("px-4 py-2 text-right text-xs font-bold", r.days > 0 ? "text-red-600" : "text-slate-500")}>
                        ...
                        <td className="px-5 py-2 text-right font-black text-slate-900">{fmt(r.balance, r.currency)}</td>
                      </tr>
                    );
                  })}
                </tbody>
