...
export interface DocumentPdfData {
  kind: "quotation" | "invoice" | "proforma_invoice";
  number: string | null;
...
export function buildDocumentPdf(d: DocumentPdfData): jsPDF {
  ...
  const titleText = d.kind === "quotation" ? "QUOTATION" : d.kind === "proforma_invoice" ? "PROFORMA INVOICE" : "INVOICE";
  ...
  rightRow(d.kind === "quotation" ? "Quote No:" : d.kind === "proforma_invoice" ? "Proforma No:" : "Invoice No:", d.number || "—");
  rightRow("Date Issued:", d.dateIssued ? new Date(d.dateIssued).toLocaleDateString("en-GB") : "—");
  rightRow(d.kind === "invoice" ? "Due Date:" : "Valid Until:", d.dueOrValidUntil ? new Date(d.dueOrValidUntil).toLocaleDateString("en-GB") : "—");
  ...
  doc.text(d.kind === "quotation" ? "Prepared For:" : d.kind === "proforma_invoice" ? "Bill To:" : "Attention To:", 14, cy);
  doc.setTextColor(0);
  cy += 6;
  doc.setFontSize(11);
  ...
  if (d.kind !== "invoice" && d.dueOrValidUntil) {
  ...
  if (d.kind === "proforma_invoice") {
    ...
    const disclaimer = doc.splitTextToSize(
      "This is a preliminary Proforma Invoice for reference purposes only. It is not a tax invoice and does not constitute a demand for payment.",
      pageWidth - 32,
    );
    doc.text(disclaimer, pageWidth / 2, y, { align: "center" });
    doc.setTextColor(0);
    doc.setFont("helvetica", "normal");
    y += 6 + disclaimer.length * 4;
  }
  ...
  if (d.kind !== "quotation" && d.termsConditions) {
    doc.setFont("helvetica", "bold");
    doc.setFontSize(9);
    doc.setTextColor(...MUTED);
  ...
  doc.setTextColor(...MUTED);
  doc.text(
    `This document was generated by the ${d.company.company_name || "company"} ${d.kind === "quotation" ? "Logistics" : "Rental"} System.`,
