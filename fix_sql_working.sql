-- WORKING VERSION - Accounting Integration
-- Run this in Supabase SQL Editor

-- ============================================
-- FUNCTION: Create Expense Journal Entry
-- ============================================
CREATE OR REPLACE FUNCTION create_expense_journal_entry(
    p_expense_id UUID,
    p_expense_number TEXT,
    p_category TEXT,
    p_amount DECIMAL,
    p_description TEXT,
    p_payment_method TEXT DEFAULT 'cash'
)
RETURNS UUID AS $$
DECLARE
    v_entry_id UUID;
    v_entry_number TEXT;
    v_expense_account TEXT;
    v_cash_account TEXT;
BEGIN
    -- Map expense category to chart of accounts
    v_expense_account := CASE UPPER(p_category)
        WHEN 'FUEL' THEN '5001'
        WHEN 'DRIVER' THEN '5002'
        WHEN 'TURNBOY' THEN '5003'
        WHEN 'TOLLS' THEN '5004'
        WHEN 'MAINTENANCE' THEN '6100'
        WHEN 'INSURANCE' THEN '6101'
        WHEN 'LICENSE' THEN '6102'
        WHEN 'TRACKING' THEN '6103'
        WHEN 'MARKETING' THEN '6200'
        WHEN 'ENTERTAINMENT' THEN '6201'
        WHEN 'RENT' THEN '6002'
        WHEN 'UTILITIES' THEN '6003'
        WHEN 'INTERNET' THEN '6004'
        WHEN 'SALARIES' THEN '6001'
        WHEN 'BANK_CHARGES' THEN '7001'
        WHEN 'INTEREST' THEN '7002'
        WHEN 'FINES' THEN '7003'
        ELSE '6001'
    END;
    
    -- Determine cash/bank account
    v_cash_account := CASE UPPER(p_payment_method)
        WHEN 'CASH' THEN '1001'
        WHEN 'MOBILE' THEN '1003'
        WHEN 'BANK' THEN '1002'
        ELSE '1001'
    END;
    
    -- Generate entry number
    v_entry_number := generate_entry_number();
    
    -- Create journal entry
    INSERT INTO journal_entries (
        entry_number, entry_date, reference_type, reference_id,
        description, total_debit, total_credit, source
    )
    VALUES (
        v_entry_number, CURRENT_DATE, 'EXPENSE', p_expense_id,
        'Expense: ' || p_description, p_amount, p_amount, 'auto'
    )
    RETURNING id INTO v_entry_id;
    
    -- Debit expense account
    INSERT INTO journal_entry_lines (
        journal_entry_id, account_code, debit_amount, credit_amount, description, line_order
    )
    VALUES (v_entry_id, v_expense_account, p_amount, 0, p_expense_number || ': ' || p_description, 1);
    
    -- Credit cash/bank account
    INSERT INTO journal_entry_lines (
        journal_entry_id, account_code, debit_amount, credit_amount, description, line_order
    )
    VALUES (v_entry_id, v_cash_account, 0, p_amount, 'Payment for ' || p_expense_number, 2);
    
    -- Post the entry
    PERFORM post_journal_entry(v_entry_id);
    
    RETURN v_entry_id;
END;
$$ LANGUAGE plpgsql;

-- ============================================
-- FUNCTION: Create Invoice Journal Entry
-- ============================================
CREATE OR REPLACE FUNCTION create_invoice_journal_entry(
    p_invoice_id UUID,
    p_invoice_number TEXT,
    p_client_name TEXT,
    p_amount DECIMAL,
    p_vat_amount DECIMAL,
    p_total_amount DECIMAL,
    p_description TEXT,
    p_revenue_account TEXT DEFAULT '4002'
)
RETURNS UUID AS $$
DECLARE
    v_entry_id UUID;
    v_entry_number TEXT;
    v_ar_account TEXT := '1100';
    v_vat_account TEXT := '2005';
BEGIN
    v_entry_number := generate_entry_number();
    
    INSERT INTO journal_entries (
        entry_number, entry_date, reference_type, reference_id,
        description, total_debit, total_credit, source
    )
    VALUES (
        v_entry_number, CURRENT_DATE, 'INVOICE', p_invoice_id,
        'Invoice ' || p_invoice_number || ' to ' || p_client_name, p_total_amount, p_total_amount, 'auto'
    )
    RETURNING id INTO v_entry_id;
    
    -- Debit Accounts Receivable
    INSERT INTO journal_entry_lines (journal_entry_id, account_code, debit_amount, credit_amount, description, line_order)
    VALUES (v_entry_id, v_ar_account, p_total_amount, 0, 'Receivable from ' || p_client_name, 1);
    
    -- Credit Revenue Account
    INSERT INTO journal_entry_lines (journal_entry_id, account_code, debit_amount, credit_amount, description, line_order)
    VALUES (v_entry_id, p_revenue_account, 0, p_amount, 'Revenue - ' || p_description, 2);
    
    -- Credit VAT Payable if VAT > 0
    IF p_vat_amount > 0 THEN
        INSERT INTO journal_entry_lines (journal_entry_id, account_code, debit_amount, credit_amount, description, line_order)
        VALUES (v_entry_id, v_vat_account, 0, p_vat_amount, 'VAT on invoice', 3);
    END IF;
    
    PERFORM post_journal_entry(v_entry_id);
    
    RETURN v_entry_id;
END;
$$ LANGUAGE plpgsql;

-- ============================================
-- FUNCTION: Process Expense (Simple Version)
-- ============================================
CREATE OR REPLACE FUNCTION process_expense_journal(p_expense_id UUID)
RETURNS UUID AS $$
DECLARE
    v_expense_number TEXT;
    v_category TEXT;
    v_amount DECIMAL;
    v_description TEXT;
    v_payment_method TEXT;
    v_journal_id UUID;
BEGIN
    -- Get expense details (will error if table doesn't exist)
    SELECT expense_number, category, amount, description, COALESCE(payment_method, 'cash')
    INTO v_expense_number, v_category, v_amount, v_description, v_payment_method
    FROM expenses WHERE id = p_expense_id;
    
    -- Check if record was found
    IF v_expense_number IS NULL THEN
        RAISE EXCEPTION 'Expense not found: %', p_expense_id;
    END IF;
    
    -- Create journal entry
    v_journal_id := create_expense_journal_entry(
        p_expense_id, v_expense_number, v_category,
        v_amount, v_description, v_payment_method
    );
    
    -- Try to update expense with journal reference
    BEGIN
        UPDATE expenses SET journal_entry_id = v_journal_id WHERE id = p_expense_id;
    EXCEPTION WHEN OTHERS THEN
        NULL; -- Ignore errors
    END;
    
    RETURN v_journal_id;
END;
$$ LANGUAGE plpgsql;

-- ============================================
-- FUNCTION: Process Invoice (Simple Version)
-- ============================================
CREATE OR REPLACE FUNCTION process_invoice_journal(p_invoice_id UUID)
RETURNS UUID AS $$
DECLARE
    v_invoice_number TEXT;
    v_client_name TEXT;
    v_amount DECIMAL;
    v_vat_amount DECIMAL;
    v_total_amount DECIMAL;
    v_description TEXT;
    v_trip_id UUID;
    v_revenue_account TEXT := '4002';
    v_journal_id UUID;
BEGIN
    -- Get invoice details
    SELECT invoice_number, client_name, amount, vat_amount, total_amount, description, trip_id
    INTO v_invoice_number, v_client_name, v_amount, v_vat_amount, v_total_amount, v_description, v_trip_id
    FROM invoices WHERE id = p_invoice_id;
    
    -- Check if record was found
    IF v_invoice_number IS NULL THEN
        RAISE EXCEPTION 'Invoice not found: %', p_invoice_id;
    END IF;
    
    -- Try to get revenue account from trip
    BEGIN
        SELECT revenue_account_code INTO v_revenue_account
        FROM trip_accounting WHERE trip_id = v_trip_id;
        IF v_revenue_account IS NULL THEN
            v_revenue_account := '4002';
        END IF;
    EXCEPTION WHEN OTHERS THEN
        v_revenue_account := '4002';
    END;
    
    -- Create journal entry
    v_journal_id := create_invoice_journal_entry(
        p_invoice_id, v_invoice_number, v_client_name,
        v_amount, v_vat_amount, v_total_amount,
        v_description, v_revenue_account
    );
    
    -- Try to update invoice with journal reference
    BEGIN
        UPDATE invoices SET journal_entry_id = v_journal_id WHERE id = p_invoice_id;
    EXCEPTION WHEN OTHERS THEN
        NULL;
    END;
    
    RETURN v_journal_id;
END;
$$ LANGUAGE plpgsql;

-- ============================================
-- STATUS
-- ============================================
SELECT 'Accounting integration functions created successfully!' as status;
SELECT 'Functions: create_expense_journal_entry, create_invoice_journal_entry, process_expense_journal, process_invoice_journal' as info;
